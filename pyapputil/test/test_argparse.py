...
if not hasattr(unittest.TestCase, "assertRegex"):
    print("Please install unittest2")
    sys.exit(1)
...
class ErrorRaisingArgumentParser(pyapputil.argutil.ArgumentParser):
...
class TestDefaultSuppress(ParserTestCase):
    ...
    argument_signatures = [
        ...
        Sig('--baz', action='store_true', default=pyapputil.argutil.SUPPRESS),
    ]
    failures = ['-x']
    successes = [
        ('', NS()),
...
class TestFileTypeRepr(TestCase):
    ...
    def test_r(self):
        type = pyapputil.argutil.FileType('r')
        self.assertEqual("FileType('r')", repr(type))

    def test_wb_1(self):
        type = pyapputil.argutil.FileType('wb', 1)
        self.assertEqual("FileType('wb', 1)", repr(type))

    def test_r_latin(self):
        type = pyapputil.argutil.FileType('r', encoding='latin_1')
        self.assertEqual("FileType('r', encoding='latin_1')", repr(type))

    def test_w_big5_ignore(self):
        type = pyapputil.argutil.FileType('w', encoding='big5', errors='ignore')
        self.assertEqual("FileType('w', encoding='big5', errors='ignore')",
                         repr(type))

    def test_r_1_replace(self):
        type = pyapputil.argutil.FileType('r', 1, errors='replace')
...
class WFile(object):
    ...
    def __eq__(self, other):
        if other not in self.seen:
            text = u'Check that file is writable.'
            if 'b' in other.mode:
...
class TestFileTypeOpenArgs(TestCase):
    """Test that open is correctly called"""

    def test_open_args(self):
        FT = pyapputil.argutil.FileType
        cases = [
        ...
        with mock.patch('pyapputil.argutil._open') as m:
            for factory, args in cases:
                factory('foo')
...
class TestActionUserDefined(ParserTestCase):
    ...
    class OptionalAction(pyapputil.argutil.Action):
    ...
    class PositionalAction(pyapputil.argutil.Action):
...
class TestActionRegistration(TestCase):
    ...
    class MyAction(pyapputil.argutil.Action):
...
class TestParentParsers(TestCase):
    ...
    def test_conflicting_parents(self):
        self.assertRaises(
            pyapputil.argutil.ArgumentError,
            pyapputil.argutil.ArgumentParser,
...
class TestMutuallyExclusiveManySuppressed(MEMixin, TestCase):
    ...
    def get_parser(self, required):
        ...
        add('--badger', action='store_false', help=pyapputil.argutil.SUPPRESS)
        add('--bladder', help=pyapputil.argutil.SUPPRESS)
        return parser
...
class MEPBase(object):
    ...
    def get_parser(self, required=None):
        ...
        parser = ErrorRaisingArgumentParser(
            prog=parent.prog, add_help=False, parents=[parent], add_debug=False)
        return parser

class MEPNoDebugBase(object):

    def get_parser(self, required=None):
        parent = super(MEPNoDebugBase, self).get_parser(required=required, add_debug=False)
        parser = ErrorRaisingArgumentParser(
            prog=parent.prog, add_help=False, parents=[parent], add_debug=False)
        return parser
...
class TestMutuallyExclusiveFirstSuppressedParent(
    MEPNoDebugBase, TestMutuallyExclusiveFirstSuppressed):
    pass
...
class TestNamespaceContainsSimple(TestCase):
    ...
    def test_empty(self):
        ns = pyapputil.argutil.Namespace()
        self.assertNotIn('', ns)
        self.assertNotIn('x', ns)

    def test_non_empty(self):
        ns = pyapputil.argutil.Namespace(x=1, y=2)
        self.assertNotIn('', ns)
        self.assertIn('x', ns)
        self.assertIn('y', ns)
...
class TestHelpWrappingShortNames(HelpTestCase):
    ...
    help = usage + '''\
        ...
        D DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD DD
        DD DD DD DD DD D
...
class TestHelpWrappingLongNames(HelpTestCase):
    ...
    argument_signatures = [
        ...
        Sig('y', metavar='y' * 25, help='YH YH' * 30),
    ]
    argument_group_signatures = [
        (Sig('ALPHAS'), [
            Sig('-a', metavar='A' * 25, help='AH AH' * 30),
            Sig('z', metavar='z' * 25, help='ZH ZH' * 30)]),
    ]
    usage = '''\
        usage: USAGE
        '''
    help = usage + '''\
        ...
        Positional arguments:
          yyyyyyyyyyyyyyyyyyyyyyyyy     YH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH YHYH
                                        YHYH YHYH YHYH YHYH YHYH YHYH YH
        ...
        ALPHAS:
          -a AAAAAAAAAAAAAAAAAAAAAAAAA  AH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH AHAH
...
class TestInvalidArgumentConstructors(TestCase):
    ...
    def test_parsers_action_missing_params(self):
        ...
        self.assertTypeError('command', action='parsers',
                             parser_class=pyapputil.argutil.ArgumentParser)
...
class TestConflictHandling(TestCase):
    ...
    def test_conflict_error(self):
        ...
        self.assertRaises(pyapputil.argutil.ArgumentError,
                          parser.add_argument, '-x')
    ...
    def test_help_version_extra_arguments(self):
        ...
        for prefix in valid_prefixes:
            for suffix in valid_suffixes:
                format_str = '%s %%s %s' % (prefix, suffix)
            self.assertPrintHelpExit(parser, format_str % '-h')
            self.assertPrintHelpExit(parser, format_str % '--help')
            self.assertRaises(AttributeError, getattr, parser, 'format_version')
...
class TestStrings(TestCase):
    ...
    def test_optional(self):
        option = pyapputil.argutil.Action(
    ...
    def test_argument(self):
        argument = pyapputil.argutil.Action(
            option_strings=[],
            dest='x',
            type=float,
            nargs='?',
    ...
    def test_parser(self):
        ...
        string = (
            ...
            "add_help=True)" % pyapputil.argutil.ArgHelpFormatter)
...
class TestNamespace(TestCase):
    ...
    def test_constructor(self):
        ...
        ns = pyapputil.argutil.Namespace(a=42, b='spam')
        self.assertEqual(ns.a, 42)
        self.assertEqual(ns.b, 'spam')

    def test_equality(self):
        ns1 = pyapputil.argutil.Namespace(a=1, b=2)
        ns2 = pyapputil.argutil.Namespace(b=2, a=1)
        ns3 = pyapputil.argutil.Namespace(a=1)
        ns4 = pyapputil.argutil.Namespace(b=2)
    ...
    def test_equality_returns_notimplemented(self):
        # See issue 21481
        ns = pyapputil.argutil.Namespace(a=1, b=2)
...
class TestEncoding(TestCase):
    ...
    def test_argparse_module_encoding(self):
        self._test_module_encoding(pyapputil.argutil.__file__)
...
class TestArgumentError(TestCase):
    ...
    def test_argument_error(self):
        msg = "my error here"
        error = pyapputil.argutil.ArgumentError(None, msg)
...
# ============================
# from pyapputil.argutil import * tests
# ============================

class TestImportStar(TestCase):

    def test(self):
        for name in pyapputil.argutil.__all__:
            self.assertTrue(hasattr(pyapputil.argutil, name))

    def test_all_exports_everything_but_modules(self):
        items = [
            name
            for name, value in vars(pyapputil.argutil).items()
            if not (name.startswith("_") or name == 'ngettext')
            if not inspect.ismodule(value)
        ]
        self.assertEqual(sorted(items), sorted(pyapputil.argutil.__all__))
