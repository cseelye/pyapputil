# This is a forked version of argparse with some improvements to integrate with the rest of pyapputil:
#   Option to automatically get default arg values from appconfig
#   Option to automatically add a debug arg with multiple levels that integrates with logutil
#   Richer help/usage display that includes if an arg is required, what its default value is, what ENV varible can change it
...
def _isidentifier(teststr):
    ...
    func = getattr(str, "isidentifier", None)
    if func:
        return func(teststr)
    else:
        import tokenize as _tokenize
        import keyword as _keyword
        return _re.match(_tokenize.Name + '$', teststr) and not _keyword.iskeyword(teststr)
...
class HelpFormatter(object):
    ...
    def __init__(self,
        ...
        if width is None:
            ...
            if not width:
                width = HelpFormatter.FALLBACK_WIDTH
            width -= 2

        if max_help_position is None:
            max_help_position = int(width * 0.3)
        ...
        self._width = min(width, HelpFormatter.MAX_WIDTH)
        ...
        flags = getattr(_re, "ASCII", 0)
        self._whitespace_matcher = _re.compile(r'\s+', flags)
    ...
    # =======================
    def format_help(self):
        help_str = self._root_section.format_help()
        if help_str:
            help_str = self._long_break_matcher.sub('\n\n', help_str)
    ...
    def _format_usage(self, usage, actions, groups, prefix):
        ...
        # if optionals and positionals are available, calculate usage
            ...
            format_func = self._format_actions_usage
            action_usage = format_func(optionals + positionals, groups)
            ...
            if len(prefix) + len(usage) > text_width:
                ...
                part_regexp = r'\(.*?\)+|\[.*?\]+|\S+'
                opt_usage = format_func(optionals, groups)
                pos_usage = format_func(positionals, groups)
    ...
    def _format_actions_usage(self, actions, groups):
        ...
        open_re = r'[\[(]'
        close_re = r'[\])]'
        text = _re.sub(r'(%s) ' % open_re, r'\1', text)
        text = _re.sub(r' (%s)' % close_re, r'\1', text)
        text = _re.sub(r'%s *%s' % (open_re, close_re), r'', text)
...
class ArgHelpFormatter(HelpFormatter):
    """Help message formatter which adds default values/ENV override and required status to argument help message"""

    def _get_help_string(self, action):
        help_str = action.help

        # Add default value to help string
        if action.default and '%(default)' not in action.help:
            if action.default is not SUPPRESS:
                defaulting_nargs = [OPTIONAL, ZERO_OR_MORE]
                if action.option_strings or action.nargs in defaulting_nargs:
                    help_str += ' (default: %(default)s)'
...
class ArgumentError(Exception):
    """An error from incorrctly creating or using an argument (optional or positional).
...
class FileType(object):
    ...
    def __call__(self, string):
        ...
        try:
            return _open(string, self._mode, self._bufsize, self._encoding,
                        self._errors)
...
class _ActionsContainer(object):
    ...
    # ====================
    def register(self, registry_name, keyname, item):
        registry = self._registries.setdefault(registry_name, {})
        registry[keyname] = item

    def _registry_get(self, registry_name, keyname, default=None):
        return self._registries[registry_name].get(keyname, default)
...
class _ArgumentGroup(_ActionsContainer):
    ...
    def __init__(self, container, title=None, description=None, **kwargs):
        ...
        super(_ArgumentGroup, self).__init__(description=description, **kwargs)
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def _parse_known_args(self, arg_strings, namespace):
        ...
        for action in self._actions:
            if action not in seen_actions:
                # This change makes it so that required arguments can use the default value if it is not specified on the command line
                # So now 'required' means either the user specified it, or there is a default for it
                if action.required and action.default is None:
                    ...
                    if (action.default is not None and
                        isinstance(action.default, _basestring) and
    ...
    def _read_args_from_files(self, arg_strings):
        ...
        for arg_string in arg_strings:
            ...
            # replace arguments referencing files with the file content
                ...
                try:
                    with _open(arg_string[1:]) as args_file:
                        arg_strings = []
    ...
    def parse_args_to_dict(self, args=None, namespace=None, allowExtraArgs=False, extraArgsKey="extra_args"):
        if allowExtraArgs:
            ...
            if hasattr(parsed, extraArgsKey):
                raise ArgumentError(extraArgsKey, "Conflicting extra args key '{}'".format(extraArgsKey))
            if extras:
    ...
    def _get_value(self, action, arg_string):
        ...
        except (ArgumentTypeError, _exceptutil.InvalidArgumentError):
    ...
    # =====================
    def print_usage(self, filehandle=None):
        if filehandle is None:
            filehandle = _sys.stdout
        self._print_message(self.format_usage(), filehandle)

    def print_help(self, filehandle=None):
        if filehandle is None:
            filehandle = _sys.stdout
        self._print_message(self.format_help(), filehandle)

    def _print_message(self, message, filehandle=None):
        if message:
            if filehandle is None:
                filehandle = _sys.stderr
            output = message
            decoder = getattr(message, "decode", None)
            if decoder:
                output = decoder("utf-8")
...
def GetFirstLine(text):
    ...
    sentences = [ sentence.strip() for sentence in text.split(".") if sentence.strip() ][0]
    firstsentence = sentences[0] if sentences else ""

    if firstline and len(firstline) < firstsentence:
        return firstline
    if firstsentence:
